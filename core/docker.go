...
func NewDockerManager(c *utils.Config, list servers.ServiceListProvider, tlsConfig *tls.Config) (*DockerManager, error) {
	...
	dclient, err := client.NewClientWithOpts(client.WithHost(c.DockerHost), client.WithAPIVersionNegotiation(), client.WithHTTPHeaders(defaultHeaders))
...
func (d *DockerManager) Start() error {
	...
	filterFunc := func(message eventtypes.Message) bool {
		if message.Type != eventtypes.ContainerEventType {
			return false
		}
		return true
	}

	eventHandler := events.NewHandler(filterFunc, events.ByAction)
